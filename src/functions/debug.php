 ...
 */
function query_set_nocache($query)
{
    return preg_replace('/SELECT(\s)/i', 'SELECT SQL_NO_CACHE$1', $query, 1);
}

/**
 * Start a timer (if no started) and get the time
 *
 * @param  string $timer The name of the timer.
 *
 * @return int           The time.
 */
function time_start($timer = 'default')
{
    static $start_time = [];
 ...
 */
function time_elapsed($timer = 'default', $human = true)
{
 ...
 */
function memory_get_usage_kb($human = true, $real_usage = false)
{
 ...
 * @return string|int       The memory usage.
 */
function memory_get_usage_mb($human = true, $real_usage = false)
{
 ...
 * @return string|int       The memory usage.
 */
function memory_get_peak_usage_kb($human = true, $real_usage = false)
{
 ...
 * @return string|int       The memory usage.
 */
function memory_get_peak_usage_mb($human = true, $real_usage = false)
{
 ...
 */
function memory_usage_format($usage, $unit, $human)
{
    if ($human) {
